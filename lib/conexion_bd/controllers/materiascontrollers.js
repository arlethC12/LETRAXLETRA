...
export const getmateria = async (req, res) => {
    ...
    try {
        // Validate and convert cve_materia to integer
        const id = parseInt(cve_materia, 10);
        if (isNaN(id)) {
            return res.status(400).json({ message: 'cve_materia debe ser un número válido' });
        }

        const { rows } = await pool.query(
            'SELECT cve_materia, nombre_materia FROM materias WHERE cve_materia = $1',
            [id]
        );
        if (rows.length === 0) {
